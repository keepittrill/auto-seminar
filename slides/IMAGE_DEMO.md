...
1. **인라인** 버튼 → `![이미지 설명](./assets/image.jpg)` 클립보드 복사
2. **배경** 버튼 → `![bg](./assets/bg.jpg)` 클립보드 복사
3. **분할** 버튼 → `![bg left:40%](./assets/left.jpg)` 클립보드 복사
4. ✏️ **MD 소스 편집** 버튼으로 에디터 열기
5. 붙여넣기(`Ctrl+V`) 후 파일명 수정
6. 💾 **.md 다운로드**로 저장 후 `slides/`에 적용

> ⚠️ 클립보드 복사는 **HTTPS** (GitHub Pages) 또는 **localhost** 환경에서만 동작합니다.
> `file://` 로컬 파일 직접 열기에서는 브라우저 보안 정책으로 동작하지 않습니다.

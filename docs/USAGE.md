...
슬라이드 뷰어의 🎨 패널 → **🖼 이미지 삽입** 버튼을 클릭하면 MD 소스 에디터가 자동으로 열리고, **현재 보고 있는 슬라이드의 끝**에 이미지 문법이 바로 삽입됩니다.
...
삽입 후 파일명(`image.jpg` 등)만 실제 파일명으로 수정하고 💾 .md 다운로드하면 됩니다.

> 에디터가 이미 열려 있는 상태에서 버튼을 클릭하면 textarea의 **커서 위치**에 삽입됩니다.

### 4.6.4 라이브 미리보기 (👁 미리보기)

MD 소스 에디터 헤더의 **👁 미리보기** 버튼을 켜면, textarea를 편집하는 동안 아래 패널에
슬라이드가 **실시간으로 렌더링**됩니다 (PowerPoint처럼 보면서 편집).

- 렌더 엔진: **Marp Core** — 미리보기를 처음 켤 때만 esm.sh CDN에서 1회 지연 로드
  (일반 열람·오프라인 동작에는 영향 없음). 네트워크가 없으면 미리보기만 비활성화됩니다.
- 미리보기는 **격리된 iframe**에 렌더되어 실제 슬라이드/테마/스위처와 충돌하지 않습니다.
- 미리보기 테마는 **빌드 시점에 확정된 그 슬라이드의 테마**로 고정됩니다.
- 입력 후 약 0.35초 debounce → 재렌더. 켬/끔 상태는 `localStorage`에 저장됩니다.

> ⚠️ **저장은 로컬에서만:** 미리보기 편집 결과는 **내 브라우저에만** 보입니다.
> 영구 반영하려면 💾 `.md` 다운로드 → `slides/`에 덮어쓰기 → `git push` → 자동 재배포.
> (보안상 브라우저에 GitHub 쓰기 토큰을 두지 않기 때문입니다.)

> ⚠️ **차이날 수 있는 항목:** mermaid 다이어그램은 미리보기에서 **코드로** 표시되고,
> 배경 이미지(`![bg]`)·분할 레이아웃은 배포본과 미세하게 다를 수 있습니다.
> 정확한 최종 모습은 배포 후 확인하세요.
...
- 이미지 도우미 버튼은 클립보드 API를 사용하지 않으므로 `file://` 로컬 환경에서도 정상 동작합니다.
